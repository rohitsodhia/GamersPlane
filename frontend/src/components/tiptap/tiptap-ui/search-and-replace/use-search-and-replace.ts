 ...
 */
export function formatResultCount(currentIndex: number | null, total: number): string {
...
export function useSearchAndReplace(config?: UseSearchAndReplaceConfig) {
	...
	const { total, currentIndex, appliedSearchTerm, caseSensitive, wholeWord, useRegex } =
		editorState;
	...
	const canReplace =
		canSearch && isEditable && hasReplaceTerm && total > 0 && currentIndex !== null;
